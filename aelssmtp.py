...
try:
    ...
    # Добавлен PySocks в список устанавливаемых пакетов
    os.system('apt -y install python3-pip; pip3 install psutil requests dnspython pyopenssl PySocks')
    import psutil, requests, dns.resolver

# Добавлен импорт socks после установки всех необходимых пакетов
import socks

# Определение функции для настройки SOCKS прокси
...
if __name__ == '__main__':
    ...
    proxy_host = input("Введите адрес SOCKS прокси: ")
    proxy_port = int(input("Введите порт SOCKS прокси: "))
    set_socks_proxy(proxy_host, proxy_port)
